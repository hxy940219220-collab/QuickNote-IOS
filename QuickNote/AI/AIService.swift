...
enum AITextAction: String, Sendable {
    ...
    var instruction: String {
        ...
        case .translate:
            "识别材料的主要语言。中文翻译成自然英文，英文或其他语言翻译成中文。网址原样保留：所有以 http:// 或 https:// 开头的 URL 不翻译、不改写、不生成读音，也不计入长度。若译文为中文且不超过 10 个汉字，另起一行附带声调的“拼音：”；多于 10 个汉字则不生成拼音。若译文为英文且不超过 10 个英文单词，另起一行附 IPA“音标：”；多于 10 个英文单词则不生成音标。只输出译文和符合条件的读音，并保留原段落结构。"
        }
    }
}
